...
//			$olduser->company_name = $request->company_name;
			...
			$user = User::withCount(['total_document','document_pdf','document_text','document_url','document_image'])->where('id',Auth::user()->id)->first();
	...
	public function negotiator_photo_update(Request $request)
	{
		...
		$users = User::withCount(['total_document','document_pdf','document_text','document_url','document_image'])->find(Auth::user()->id);
